...
func NewHttpChunkedReader(log *slog.Logger, client *http.Client, audioUrl string) (_ *HttpChunkedReader, err error) {
	...
	for i := int64(0); i < totalChunks; i++ {
		r.chunks[i] = &chunkItem{Cond: sync.NewCond(&sync.Mutex{})}
	}
...
func (r *HttpChunkedReader) downloadChunk(idx int) (*http.Response, error) {
	return backoff.Retry(context.Background(), func() (*http.Response, error) {
		...
		return resp, nil
	}, backoff.WithBackOff(backoff.NewConstantBackOff(time.Second)), backoff.WithMaxTries(3))
}
...
func (r *HttpChunkedReader) fetchChunk(idx int) ([]byte, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed downloading chunk %d: %w", idx, err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed reading chunk %d: %w", idx, err)
	}
...
func (r *HttpChunkedReader) Size() int64 { return r.len }

func (r *HttpChunkedReader) Url() *url.URL { return r.url }
