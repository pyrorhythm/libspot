...
// Decryptor decrypts a Spotify audio stream on the fly. The stream is
// AES-128-CTR encrypted with a fixed base IV; the counter is derived from the
// absolute byte offset, so reads can start at any position.
